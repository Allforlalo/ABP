...
@section('content')
    ...
    <div class="card bg-dark text-white">
        <div class="card-body">
            <form novalidate action="{{ route('pedidos_bebidas.update', $pedidoBebida->id_pedido_bebida) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label class="form-label">Nombre</label>
                    <input type="text" name="nombre" class="form-control" value="{{ $pedidoBebida->nombre }}">
                </div>
                <div class="mb-3">
                    <label class="form-label">Precio</label>
                    <input type="number" name="precio" class="form-control" value="{{ $pedidoBebida->precio }}">
                </div>
                <div class="mb-3">
